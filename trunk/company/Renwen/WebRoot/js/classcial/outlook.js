...
function b_showButton(nr) {
	...
	// this.aktPanel=nr;
	l = this.btns.length;
	var pHei = this.objf.style.height.substring(0,
			this.objf.style.height.length - 2);
	...
	for (i = 0; i < l; i++) {
		if (i > nr) {
			var objTop = this.btns[i].obj.style.top.substring(0,
					this.btns[i].obj.style.top.length - 2);
			if (this.btns[i].objf != null) {
				var objfTop = this.btns[i].objf.style.top.substring(0,
						this.btns[i].objf.style.top.length - 2);
			}
			if (bl) {
				this.btns[i].obj.style.top = parseInt(objTop)
						+ this.btns[nr].btns.length * 20;
				if (this.btns[i].objf != null) {
					this.btns[i].objf.style.top = parseInt(objfTop)
							+ this.btns[nr].btns.length * 20;
				}
				this.objf.style.height = parseInt(pHei)
						+ this.btns[nr].btns.length * 20;
			} else {
				this.btns[i].obj.style.top = parseInt(objTop)
						- this.btns[nr].btns.length * 20;
...
function b_mOver_ns4(nr) {
	...
	l.write("onMousedown='" + this.v + ".mDown(" + nr + ")'><img src='"
			+ this.img[nr]);
...
function b_mOut_ns4(nr) {
	...
	l.write("onmouseOut='" + this.v + ".mOut(" + nr + ")'><img src='"
			+ this.img[nr]);
	l.write("' border=0></A></SPAN><Font size=2 Face=Arial color=white>");
	l.write(this.lbl[nr] + "</FONT><BR><BR>");
	l.close();
...
function b_mDown_ns4(nr) {
	...
	l.write("<A href='#' onmouseOver='" + this.v + ".mOver(" + nr + ")' ");
	l.write("onmouseOut='" + this.v + ".mOut(" + nr + ")' onMouseup='"
			+ this.act[nr]);
...
function createPanel(name, caption) {
	...
	this.clear = b_clear;
	if (bt.ns4)
		this.showButton = b_showButton_ns4; // make a panel visible (ns4)
	else
		this.showButton = b_showButton; // reset all buttons
...
function b_writeStyle() {
	...
	document
			.write('	background:url(images/classcial/013.gif) transparent no-repeat left top;');
	document.write('}');

	document.write('.button Table tr td {');
	document
			.write('    font-size:14px;letter-spacing:1px;font-weight:bold;color:#8B6A33;FILTER:dropshadow(color=#C9C9C9, offx=1, offy=1, positive=1);');
...
function b_draw() {
	...
	if (bt.ns4) { // draw OutlookBar for ns4
		// OutlookBar layer..
		document.write('<layer bgcolor=' + this.bgcolor + ' name=' + this.name
				+ ' left=');
		document
				.write(this.xpos + ' top=' + this.ypos + ' width=' + this.width);
		document.write(' clip="0,0,' + this.width + ',' + this.height + '">');
		// one layer for every panel...
		for (i = 0; i < this.panels.length; i++) {
			document.write('<Layer name=' + this.name + '_panel' + i
					+ ' width=' + this.width);
			...
			for (j = 0; j < this.panels[i].img.length; j++) {
				...
				document.write(".rst(" + j + ")' onmouseOver='"
						+ this.panels[i].v);
				document.write(".mOver(" + j + ")'><img src='"
						+ this.panels[i].img[j]);
			...
			document.write('<A class=noLine href="javascript:' + this.v
					+ '.showPanel(');
			document.write(i + ');" onmouseOver="' + this.panels[i].v
					+ '.clear();">');
			document.write('<Font Color=black class=noLine>'
					+ this.panels[i].caption);
			document.write('</Font></A></Layer>');
			// two layers for scroll-up -down buttons
			document.write('<Layer visibility=hide top=40 left='
					+ (this.width - 20));
			document.write('><A href="#" onClick="' + this.panels[i].v
					+ '.down(16);" ');
			document.write('onmouseOver="' + this.panels[i].v
					+ '.clear();"><img ');
			document.write('width=16 height=16 src=arrowup.gif border=0>');
			document.write('</A></LAYER><Layer top=');
			document.write((this.height - (this.panels.length) * 20)
					+ '<Layer top=');
			document
					.write((this.height - (this.panels.length) * 20) + ' left=');
			document.write((this.width - 20) + '><A href="#" onClick="');
			document.write(this.panels[i].v + '.up(16);" onmouseOver="');
			document.write(this.panels[i].v
					+ '.clear();"><img width=16 height=16 ');
		...
		document.write(this.xpos + '; top:' + this.ypos + '; width:'
				+ this.width);
		document.write('; height:' + this.height + '; background:'
				+ this.background)
		document.write('; clip:rect(0,' + this.width + ',' + this.height
				+ ',0)">');
		...
		for (i = 0; i < this.panels.length; i++) {
			...
			document.write('; width:' + this.width + '; height:' + h
					+ '; clip:rect(0px, ');
			document.write(this.width + 'px, ' + h + 'px, 0px); background:');
			document.write(this.background + ';">')
			t = t + 20;
			// one layer to host the panel buttons
			document.write('<div id=' + this.name + '_panel' + i);
			document
					.write('_f Style="position:absolute; left:0; top:28; width:');
			document.write(this.width + '; height:');
			document.write((this.panels[i].btns.length * this.buttonspace));
			document.write('; background:' + this.background + ';">');
			mtop = 0;
			...
			for (j = 0; j < this.panels[i].btns.length; j++) {
				...
				if (bt.op5) {
					 ...
					 * '); document.write('visibility:hidden; left:0;
					 * width:'+this.width); document.write('; top:'+mtop+';
					 * text-align:center;">'); document.write('<img
					 * src='+this.panels[i].img[j]); document.write(' border=0
					 * class=imgbin onmouseUp=\''+this.panels[i].v);
					 * document.write('.mOver('+j+');'+this.panels[i].act[j]);
					 * document.write(';\'
					 * onmouseOut="'+this.panels[i].v+'.mOut('+j);
					 * document.write(');"><a href="'+this.panels[i].act[j]+'"
					 * target="'+this.panels[i].targ[j]+'"><span
					 * class="unSelected"
					 * onclick="remindState(this);">'+this.panels[i].lbl[j]+'</span></a></DIV>');
					 * 
					 * document.write('<DIV id='+this.name+'_panel'+i+'_b'+j+'1
					 * class=imgB'); document.write(' Style="position:absolute;
					 * visibility:hidden; '); document.write('left:0;
					 * width:'+this.width+'; top:'+mtop); document.write(';
					 * text-align:center;">'); document.write('<img
					 * src='+this.panels[i].img[j]); document.write(' border=0
					 * class=imgbout onmouseDown="'+this.panels[i].v);
					 * document.write('.mDown('+j+');"
					 * onmouseUp=\''+this.panels[i].v);
					 * document.write('.mOver('+j+');'+this.panels[i].act[j]);
					 * document.write(';\'
					 * onmouseOut="'+this.panels[i].v+'.mOut('+j);
					 * document.write(');"><a href="'+this.panels[i].act[j]+'"
					 ...
					 * '); document.write('visibility:visible; left:0;
					 * width:'+this.width); document.write('; top:'+mtop+';
					 * text-align:center;">'); document.write('<img
					 * src='+this.panels[i].img[j]+' class=imgnob border=0 ');
					...
					// caidankongzhidedaima
					if (this.panels[i].btns[j].btns != undefined
							&& this.panels[i].btns[j].btns.length > 0) {
						document.write('<DIV id=' + this.name + '_panel' + i
								+ '_b' + j + ' class=imgB ');
						document
								.write('Style="position:absolute; left:0; width:'
										+ this.width);
						document
								.write('; top:' + mtop + '; text-align:left;">');
						document.write('<table width="95%" border="0">');
						document.write('<tr><td width="15%" align="right">');
						document.write('<img onclick="javascript:'
								+ this.panels[i].v + '.showButton(' + j
								+ ');" style="cursor:hand" id="' + this.name
								+ '_panel' + i + '_b' + j + '_m" src='
								+ this.panels[i].btns[j].img + ' border=0>');
						document.write('</td>');
						document.write('<td width="85%">');
						document
								.write('<span class="unSelected" style="cursor:hand;" onclick="javascript:'
										+ this.panels[i].v
										+ '.showButton('
										+ j
										+ ');">'
										+ this.panels[i].btns[j].title
										+ '</span>');
						...
						document.write('<div id=' + this.name + '_panel' + i
								+ '_b' + j);
						document
								.write('_f Style="visibility:hidden; position:absolute; left:5; top:'
										+ (mtop + 20) + '; width:');
						document.write(this.width + '; height:');
						document
								.write((this.panels[i].btns[j].btns.length * this.buttonspace));
						document.write('; background:' + this.background
								+ ';">');
						sbtop = 0;
						for (var k = 0; k < this.panels[i].btns[j].btns.length; k++) {
							document.write('<DIV id=' + this.name + '_panel'
									+ i + '_b' + j + ' class=imgB ');
							document
									.write('Style="position:absolute; left:0; width:'
											+ this.width);
							document.write('; top:' + sbtop
									+ '; text-align:left;">');
							document.write('<table width="95%" border="0">');
							document
									.write('<tr><td width="15%" align="right">');
							document.write('<img src='
									+ this.panels[i].btns[j].btns[k].img
									+ ' border=0>');
							document.write('</td>');
							document.write('<td width="85%">');
							document
									.write('<a href="'
											+ this.panels[i].btns[j].btns[k].url
											+ '" target="'
											+ this.panels[i].btns[j].btns[k].target
											+ '"><span class="unSelected" onclick="remindState(this);">'
											+ this.panels[i].btns[j].btns[k].title
											+ '</span></a>');
						...
						document.write('<table width="95%" border="0">');
						document.write('<tr><td width="15%" align="right">');
						document.write('<img src=' + this.panels[i].btns[j].img
								+ ' border=0>');
						document.write('</td>');
						document.write('<td width="85%">');
						document
								.write('<a href="'
										+ this.panels[i].btns[j].url
										+ '" target="'
										+ this.panels[i].btns[j].target
										+ '"><span class="unSelected" onclick="remindState(this);">'
										+ this.panels[i].btns[j].title
										+ '</span></a>');
						document.write('</a></td></tr></table></DIV>');
			...
			if (!bt.op5) {
				document.write('<DIV id=' + this.name + '_panel' + i
						+ '_c class=button ');
				document.write('onClick="javascript:' + this.v + '.showPanel('
						+ i);
				document
						.write(');" style="position:absolute; left:0; top:0; width:');
				document.write((this.width - c) + '; height:' + (20 - c)
						+ ';"><table><tr><td ');
				document.write('onClick="' + this.v + '.showPanel(' + i
						+ ');this.blur();');
		...
		// Opera bug (Clip!)
		// op5 doesn't support layer clipping! so use top layers for panel
		// caption
		// and two top layers with background-color like page color to hide
		// panel content outside of the outlookbar.
		// op5 unterst?tzt kein Clip bei Layers! darum erzeugen wir drei top
		// level
		...
		if (bt.op5) {
			...
			for (i = 0; i < this.panels.length; i++) {
				document.write('<DIV id=' + this.name + '_panel' + i);
				document.write('_c class=button onmouseOver="'
						+ this.panels[i].v);
				document.write('.clear();" onClick="' + this.v + '.showPanel('
						+ i);
				document
						.write(');" style="position:absolute; left:0; top:0; width:');
				document.write((this.width - c) + '; height:' + (20 - c)
				...
				document.write('onClick="' + this.v + '.showPanel(' + i
						+ ');this.blur();');
				document
						.write('return false;" class=noLine><FONT color=black ');
			...
			// ');
			// document.write('height:300; width:'+this.width+';
			// background-color:');
	...
	for (i = 0; i < this.panels.length; i++) {
		...
		for (j = 0; j < this.panels[i].btns.length; j++) {
			this.panels[i].btns[j].obj = getObj(this.name + '_panel' + i + '_b'
					+ j);
			this.panels[i].btns[j].objf = getObj(this.name + '_panel' + i
					+ '_b' + j + '_f');
			this.panels[i].btns[j].objm = getObj(this.name + '_panel' + i
					+ '_b' + j + '_m');
		}
		if (!bt.ns4) {
...
function b_resize(x, y, width, height) {
	...
	if (bt.ns4)
		...
		if (bt.op5 && (width != this.width))
			...
			o.style.clip = 'rect(0px ' + this.width + 'px ' + this.height
					+ 'px 0px)';
			...
			if (this.height > ((this.panels.length) * 20 + 32 * 2)) {
				...
				return true;
				// h=20+32*2;
				// parent.fraDeeptree.scrolling='yes';
			...
			for (i = 0; i < this.panels.length; i++) {
				...
				this.panels[i].objm2.style.top = (this.height - (this.panels.length)
						* 20);
				...
				this.panels[i].obj.style.clip = 'rect(0px ' + this.width
						+ 'px ' + h + 'px 0px)';
...
function createOutlookBar(name, x, y, width, height, bgcolor, pagecolor) {
	...
	this.background = "url(images/classcial/bg.gif)";
	this.background1 = "images/classcial/bg.gif";
