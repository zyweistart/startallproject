...
<script type="text/javascript" src="${pageContext.request.contextPath}/js/system/tree_model.js"></script>
    ...
    <div style="overflow: auto">
        <script language="JavaScript">
            ...
            if(document.getElementById) {
                ...
                var i = 0;
                var j = i+1;
                var root = new WebFXTree('<fmt:message key="model.treeMenu" />');
                treeitem[0] =  new WebFXTreeItem(0,-1,'<fmt:message key="model.topMenu" />');
                <c:forEach items="${models}" var="model">
                    treeitem[j++] =  new WebFXTreeItem('${model.id}','${model.parentId}',"${model.name}");
                </c:forEach>
                root.add(treeitem[0]);
                getTree_new();
                document.write(root);}
