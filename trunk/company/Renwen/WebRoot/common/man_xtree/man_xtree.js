...
function WebFXTreeItem(nodeid, parentid, sText, sAction, sTarget, eParent,
		sIcon, sOpenIcon) {
	this.nodeid = nodeid;
	this.target = sTarget;
	...
	if (this.parentid != -999) {
		var str = "<div id=\""
				...
				+ ((webFXTreeHandler.behavior == 'classic' && this.open)
						...
						: this.icon)
				+ "\" onclick=\"webFXTreeHandler.select(this);\">"
				+ "<input class=\"webfx-tree-icon\" type=\"checkbox\" onclick =\"chkboxselect("
				+ this.nodeid
				+ ","
				+ this.parentid
				+ ")\" name=\"chkbox"
...
function addchild(parent, parentid) {
	for (var i = 0; i < treeitem.length; i++) {
		if (treeitem[i].parentid == parentid) {
			parent.add(treeitem[i]);
		}
	}
}
...
function getTree() {
	...
	for (var i = 0; i < treeitem.length; i++) {
		if (treeitem[i].parentid == tree.nodeid) {
			tree.add(treeitem[i]);
		}
	}
	for (var i = 0; i < treeitem.length; i++) {
		addchild(treeitem[i], treeitem[i].nodeid);
	}
	return root;
}
...
function getMulTree() {
	...
	for (var i = 0; i < treeitem.length; i++) {
		if (treeitem[i].parentid != -1) {
			addparent(treeitem[i], treeitem[i].parentid);
		}
	}
	return root;
}
...
function addparent(child, parentid) {
	for (var i = 0; i < treeitem.length; i++) {
		if (treeitem[i].nodeid == parentid) {
			treeitem[i].add(child);
			continue;
		}
	}
}

function getTree_new() {
	root.setBehavior('classic');
...
function chkboxselect(x, y) {
	if (document.all["chkbox" + x].checked) {
		/* flag=true; */
		flag = false;
	} else {
		flag = false;
...
function chkchild(id, flag) {
	for (var i = 0; i < treeitem.length; i++) {
		if (treeitem[i].nodeid == id) {
			var no = i;
			break;
		}
	}

	for (var i = 0; i < treeitem[no].childNodes.length; i++) {
		document.all["chkbox" + treeitem[no].childNodes[i].nodeid].checked = flag;
		if (treeitem[no].childNodes[i].childNodes.length > 0) {
			chkchild(treeitem[no].childNodes[i].nodeid, flag);
		}
	}
}

function chkparent(cid, pid, flag) {
	for (var i = 0; i < treeitem.length; i++) {
		if (treeitem[i].nodeid == cid) {
			var no = i;
			break;
		}
	}

	for (var i = 0; i < treeitem[no].parentNode.childNodes.length; i++) {
		if (document.all["chkbox"
				+ treeitem[no].parentNode.childNodes[i].nodeid].checked) {
			/* flag=true; */
			flag = false;
			break;
		}
	...
	if (treeitem[no].parentNode.nodeid != null) {
		document.all["chkbox" + treeitem[no].parentNode.nodeid].checked = flag;
	}
	if (pid != -1 && treeitem[no].parentNode.parentNode.nodeid != null) {
		chkparent(treeitem[no].parentNode.nodeid,
				treeitem[no].parentNode.parentNode.nodeid, flag);
	}
}
