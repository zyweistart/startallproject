...
public class HtmlView extends HtmlBuilder implements View {
    ...
    private void rowStyleAndJavascript(TableModel model,
    ...
    public void toolbarPlacement(TableModel model) {
        ...
        if (!showPagination
                && !showExports
                && StringUtils.isBlank(model.getTableHandler().getTable().getTitle())) {
            return;
        }
    ...
    public void toolbar(TableModel model) {
        ...
        table(2).border("0").cellPadding("0").cellSpacing("1").styleClass(
                TOOLBAR).close();
    ...
    private void toolbarFormStart(TableModel model) {
        ...
        String hiddenFields = TreeRegistryUtils.getFormHiddenFields(model,
    ...
    public void toolbarPaginationIcons(TableModel model) {
        ...
        if (!BuilderUtils.isFirstPageEnabled(page)) {
            String firstPageImage = BuilderUtils.getImage(model,
                    FIRST_PAGE_DISABLED);
            img(firstPageImage);
        } else {
            String firstPageImage = BuilderUtils.getImage(model, FIRST_PAGE);
            String firstPageTooltip = model.getMessages().getMessage(
                    BuilderConstants.TOOLBAR_FIRST_PAGE_TOOLTIP);
        ...
        if (!BuilderUtils.isPrevPageEnabled(page)) {
            String prevPageImage = BuilderUtils.getImage(model,
                    PREV_PAGE_DISABLED);
            img(prevPageImage);
        } else {
            String prevPageImage = BuilderUtils.getImage(model, PREV_PAGE);
            String prevPageTooltip = model.getMessages().getMessage(
        ...
        if (!BuilderUtils.isNextPageEnabled(page, totalPages)) {
            String nextPageImage = BuilderUtils.getImage(model,
                    NEXT_PAGE_DISABLED);
            img(nextPageImage);
        } else {
            String nextPageImage = BuilderUtils.getImage(model, NEXT_PAGE);
            String nextPageTooltip = model.getMessages().getMessage(
        ...
        if (!BuilderUtils.isLastPageEnabled(page, totalPages)) {
            String lastPageImage = BuilderUtils.getImage(model,
                    LAST_PAGE_DISABLED);
            img(lastPageImage);
        } else {
            String lastPageImage = BuilderUtils.getImage(model, LAST_PAGE);
            String lastPageTooltip = model.getMessages().getMessage(
                    BuilderConstants.TOOLBAR_LAST_PAGE_TOOLTIP);
            paginationImage(model, totalPages, lastPageImage, lastPageTooltip);
        }
        tdEnd();
    }

    public void paginationImage(TableModel model, int page, String image,
            String tooltip) {
        a();
        quote();
        ...
        question().append(model.getTableHandler().prefixWithTableId()).append(
                TableConstants.PAGE).equals().append("" + page);
        append(TreeRegistryUtils.getURLParameterString(model, true, true,
                false, true));
        quote().close();
        img(image, tooltip);
        aEnd();
    ...
    private void exportImage(TableModel model, Export export) {
        ...
        question().append(TableConstants.EXPORT_TABLE_ID).equals().append(
                model.getTableHandler().getTable().getTableId()).ampersand().append(
                model.getTableHandler().prefixWithTableId()
                + TableConstants.EXPORT_VIEW).equals().append(
                export.getView()).ampersand().append(
                model.getTableHandler().prefixWithTableId()
                + TableConstants.EXPORT_FILE_NAME).equals().append(export.getFileName());
        append(TreeRegistryUtils.getURLParameterString(model, true, true, true,
                true));
        quote();
        close();
    ...
    public void statusBar(TableModel model) {
        ...
        if (model.getTableHandler().getTable().isFilterable()) {
            ...
            if (StringUtils.isNotEmpty(imageSearch)) {
                String searchTooltip = model.getMessages().getMessage(
                        BuilderConstants.TOOLBAR_FILTER_TOOLTIP);
            ...
            if (StringUtils.isNotEmpty(imageClear)) {
                String clearTooltip = model.getMessages().getMessage(
                        BuilderConstants.TOOLBAR_CLEAR_TOOLTIP);
...
//		form();
        ...
        input("hidden").name(
                model.getTableHandler().prefixWithTableId()
                + TableConstants.FILTER + TableConstants.ACTION).close();
    ...
    public void totals(TableModel model) {
        ...
        if (calcTitle != null && calcTitle.length > 0) {
            tr(1).close();
            td(2).styleClass(TABLE_TOTAL_TITLE).colSpan(
                    "" + model.getColumnHandler().columnCount()).close();
        ...
        for (Iterator iter = model.getColumnHandler().getColumns().iterator(); iter.hasNext();) {
            ...
            if (column.isCalculated()) {
                ...
                for (int i = 0; i < calcValues.length; i++) {
                    ...
                    append(ExtremeUtils.formatNumber(column.getFormat(), value,
                            model.getLocale()));
