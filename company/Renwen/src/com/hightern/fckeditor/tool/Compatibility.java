...
import com.hightern.fckeditor.FCKeditor;
...
public class Compatibility {
	...
	public static boolean check(final String userAgentString) {
		...
		if (userAgentStr.indexOf("opera") < 0 && userAgentStr.indexOf("mac") < 0
				&& Compatibility.getBrowserVersion(userAgentStr, ".*msie ([\\d]+.[\\d]+).*") >= 5.5f) { return true; }
		
		// for all gecko based browsers
		if (Compatibility.getBrowserVersion(userAgentStr, ".*rv:([\\d]+.[\\d]+).*") > 1.7f) { return true; }
		
		// Opera 9.5+
		if (Compatibility.getBrowserVersion(userAgentStr, "opera/([\\d]+.[\\d]+).*") >= 9.5f
				|| Compatibility.getBrowserVersion(userAgentStr, ".*opera ([\\d]+.[\\d]+)") >= 9.5f) { return true; }
