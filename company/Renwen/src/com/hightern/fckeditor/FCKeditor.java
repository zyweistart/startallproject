 ...
 */
package com.hightern.fckeditor;
...
import com.hightern.fckeditor.tool.XHtmlTagTool;

/**
 * Java representation of the <a href="http://docs.fckeditor.net/FCKeditor_2.x/Developers_Guide/Server_Side_Integration#FCKeditor_Creator" target="_blank">FCKeditor</a>. This representation reflects
 * the editor in an object-oriented way. It can be configured as any other JavaBean type class. The final output of this class is HTML code.<br />
 * <strong>Note:</strong> It's your responsibility to supply reasonable and valid values, only {@code request}, {@code instanceName} and {@code inputName} will be checked for sanity.
 * 
...
public class FCKeditor {
	 ...
	 * @param inputName
	 *            the name for the underlying input element. See {@link #setInputName(String)} for details.
	 * @param width
	 ...
	 * @param value
	 *            the HTML markup of this editor. Markup will be properly escaped.
	 * @param basePath
	 *            the base path of this editor, absolute to the context
	 * @throws IllegalArgumentException
	 *             if instanceName is empty or not a valid XHTML id
	 */
	public FCKeditor(HttpServletRequest request, String instanceName, String inputName, String width, String height, String toolbarSet, String value,
			String basePath) {
	 ...
	 *             if instanceName is empty or not a valid XHTML id
	 */
	public FCKeditor(final HttpServletRequest request, final String instanceName, final String width, final String height, final String toolbarSet,
			final String value, final String basePath) {
	...
	public void setInstanceName(final String instanceName) {
		...
		if (!instanceName.matches("\\p{Alpha}[\\p{Alnum}:_.-]*")) { throw new IllegalArgumentException(
				"instanceName must be a valid XHTML id containing only \"\\p{Alpha}[\\p{Alnum}:_.-]*\""); }
		this.instanceName = instanceName;
	}
	
	/**
	 * Sets the name for the underlying input element. Empty strings will be ignored and field will be reset to {@code instanceName}.
	 * 
	 * @param inputName
	...
	/**
	 * Sets the base path of this editor. The base path reflects the location of the editor files absolute to the context root <i>not</i> the server root.
	 * 
	 * @param basePath
	 *            the base path of this editor, absolute to the context
	 */
	...
	/**
	 * Returns a configuration option. See {@link FCKeditorConfig} for more details.
	 * 
	 * @param name
	...
	/**
	 * Sets a configuration option. See {@link FCKeditorConfig} for more details.
	 * 
	 * @param name
	...
	/**
	 * Escapes base XML entities as specified <a href="http://en.wikipedia.org/wiki/Xml#Entity_references">here</a>.
	 * 
	 * @param str
	...
	/**
	 * Creates the HTML representation of this editor instance. First of all, this method determines whether the request browser is supported. According to the result an appropriate HTML
	 * representation is assembled and returned.
	 * 
	...
	public String toString() {
		...
		final String encodedValue = escapeXml(value);
		
		/*
		 * We have to reset inputName to make sure that a potentially reassigned instanceName has been propagated to empty field.
		 */
		setInputName(inputName);
		...
		if (Compatibility.isCompatibleBrowser(request)) {
			...
			// create config HTML
			final String configStr = fckConfig.getUrlParams();
			if (Utils.isNotEmpty(configStr)) {
				strEditor.append(createInputForVariable(instanceName.concat("___Config"), null, configStr));
			}
			
			// create IFrame
			final StringBuffer editorLink = new StringBuffer(request.getContextPath());
