...
function b_writeStyle() {
	...
	document
			.write('	background:url(images/default/013.gif) transparent no-repeat left top;');
	document.write('}');

	document.write('.button Table tr td {');
	document
			.write('    font-size:14px;letter-spacing:1px;font-weight:bold;color:#233E75;FILTER:dropshadow(color=#ffffff, offx=1, offy=1, positive=1);');
...
function b_draw() {
	...
	if (bt.ns4) { // draw OutlookBar for ns4
		...
		document.write(this.xpos + '; top:' + this.ypos + '; width:'
				+ this.width);
		document.write('; height:' + this.height + '; background-color:'
				+ this.bgcolor)
		...
		for (i = 0; i < this.panels.length; i++) {
			...
			document.write('; width:' + this.width + '; height:' + h
					+ '; clip:rect(0px, ');
			document.write(this.width + 'px, ' + h
					+ 'px, 0px); background-color:');
