...
var webFXTreeConfig = {
	rootIcon : '${pageContext.request.contextPath}/common/xtree/images/foldericon.png',
	openRootIcon : '${pageContext.request.contextPath}/common/xtree/images/openfoldericon.png',
	folderIcon : '${pageContext.request.contextPath}/common/xtree/images/foldericon.png',
	openFolderIcon : '${pageContext.request.contextPath}/common/xtree/images/openfoldericon.png',
	fileIcon : '${pageContext.request.contextPath}/common/xtree/images/file.png',
	iIcon : '${pageContext.request.contextPath}/common/xtree/images/I.png',
	lIcon : '${pageContext.request.contextPath}/common/xtree/images/L.png',
	lMinusIcon : '${pageContext.request.contextPath}/common/xtree/images/Lminus.png',
	lPlusIcon : '${pageContext.request.contextPath}/common/xtree/images/Lplus.png',
	tIcon : '${pageContext.request.contextPath}/common/xtree/images/T.png',
	tMinusIcon : '${pageContext.request.contextPath}/common/xtree/images/Tminus.png',
	tPlusIcon : '${pageContext.request.contextPath}/common/xtree/images/Tplus.png',
	blankIcon : '${pageContext.request.contextPath}/common/xtree/images/blank.png',
...
function WebFXTreeAbstractNode(id, sText, sAction) {
	this.childNodes = [];
	// this.id = webFXTreeHandler.getId();
	this.id = webFXTreeHandler.getId(id);
...
WebFXTreeAbstractNode.prototype.toggle = function() {
	if (this.folder) {
		if (this.open) {
			...
			this.expand();
			if (webFXTreeConfig.usePlus && this.childNodes.length == 0) {
				this.focus();
...
WebFXTree.prototype.removeNode = function() {
}

WebFXTree.prototype.modifyNode = function() {
}
...
WebFXTree.prototype.toString = function() {
	var str = "<div id=\""
			...
			+ ((webFXTreeHandler.behavior == 'classic' && this.open)
					? this.openIcon
					: this.icon)
			+ "\" onclick=\"webFXTreeHandler.select(this);\" style=\"cursor:hand\">"
			+ "<a href=\""
			+ this.action
			+ "\" id=\""
			+ this.id
			...
			+ this.text + "</a></div>" + "<div id=\"" + this.id
...
function WebFXTreeItem(id, sText, sAction, eParent, sIcon, sOpenIcon) {
	this.base = WebFXTreeAbstractNode;
	this.base(id, sText, sAction);
	/* Defaults to close */
	if (webFXTreeConfig.usePersistence) {
...
WebFXTreeItem.prototype.removeNode = function() {
	var iconSrc = document.getElementById(this.id + '-plus').src;
	var parentNode = this.parentNode;
	var prevSibling = this.getPreviousSibling(false);
	var nextSibling = this.getNextSibling(true);
	var folder = this.parentNode.folder;
	var last = ((nextSibling) && (nextSibling.parentNode) && (nextSibling.parentNode.id == parentNode.id))
			? false
	 ...
	 * if (parentNode.childNodes.length == 0) {
	 * document.getElementById(parentNode.id + '-cont').style.display = 'none';
	 * parentNode.doCollapse(); parentNode.folder = false; parentNode.open =
	 * false; } if (!nextSibling || last) { parentNode.indent(null, true, last,
	 * this._level, parentNode.childNodes.length); } if ((prevSibling ==
	 * parentNode) && !(parentNode.childNodes.length)) { prevSibling.folder =
	 * false; prevSibling.open = false; iconSrc =
	 * document.getElementById(prevSibling.id + '-plus').src; iconSrc =
	 * iconSrc.replace('minus', '').replace('plus', '');
	 * document.getElementById(prevSibling.id + '-plus').src = iconSrc;
	 * document.getElementById(prevSibling.id + '-icon').src =
	 * webFXTreeConfig.fileIcon; } if (document.getElementById(prevSibling.id +
	 * '-plus')) { if (parentNode == prevSibling.parentNode) { iconSrc =
	 * iconSrc.replace('minus', '').replace('plus', '');
	 * document.getElementById(prevSibling.id + '-plus').src = iconSrc; } }
	 */
}
WebFXTreeItem.prototype.modifyNode = function(oItem) {
	document.getElementById(this.id + '-anchor').innerText = oItem;
}
...
WebFXTreeItem.prototype.toString = function(nItem, nItemCount) {
	...
	var str = "<div id=\""
			...
			+ ((this.folder) ? ((this.open) ? ((this.parentNode._last)
					...
					: webFXTreeConfig.tIcon))
			+ "\" onclick=\"webFXTreeHandler.toggle(this);\" style=\"cursor:hand\">"
			+ "<img id=\""
			+ this.id
			+ "-icon\" class=\"webfx-tree-icon\" src=\""
			+ ((webFXTreeHandler.behavior == 'classic' && this.open)
